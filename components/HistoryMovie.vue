<template>
    <div class="movie-card">
        <div class="movie-card__img">
            ...
            <NuxtLink
                :to="movie?.is_mobile_only == true ? '/only-on-mobile' : authStore.token == null ? '/auth/login/' : movie?.content_type == 'movie' ? `/movie/${movie?.slug}/` : `/series/${movie?.id}?page=1`">
                <img class="img"
        ...
        <div class="movie-card__text-wrapper">
            <NuxtLink class="movie-card__title"
                :to="movie?.is_mobile_only == true ? '/only-on-mobile' : authStore.token == null ? '/auth/login/' : movie?.content_type == 'movie' ? `/movie/${movie?.slug}/` : `/series/${movie?.id}?page=1`">
                {{
...
async function getLastPosition() {
    ...
    if (movie?.content_id) {
        const data: any = await movieServices.getWatchTime(store.userInfo?.id, movie?.content_id, movie?.content_type)
