...
async function getWatchTime() {
    const res = await movieServices.getWatchTime(store.userInfo?.id, props.item?.id, props.item?.content_type)
    watcheTime.value = res
}
if (props.item?.id && store.userInfo?.id) {
    await getWatchTime()
}
onMounted(() => {
    const el: any = document.querySelector('.video-js');
    var player: any = videojs(el, {
        fluid: true,
