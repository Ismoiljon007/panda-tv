            ...
            <form action="#" @submit.prevent="enterCode()" class="login-form">
                <div class="mt-2 input-code-wr translate-x-[-10px] flex items-center">
                    <v-otp-input ref="otpInput" v-model:value="bindModal" input-classes="otp-input" separator="-"
                        :num-inputs="6" :should-auto-focus="true" input-type="number"
                ...
                <div class="login-btns">
                    ...
                    <div class="time-reapet">
                        ...
                        <button v-if="sendReapet" type="button" @click="sendReapetFunc()">Qaytadan
                            yuborish</button>
                    </div>
                </div>
            </form>
...
async function enterCode() {
    ...
    try {
        if (data?.access_token) {
            ...
            useToast().warning("Kiritgan ko'dingizda xatolik bor yana bir bor urinib ko'ring!")
        }
    } catch (error) {
        useToast().warning("Kiritgan ko'dingizda xatolik bor yana bir bor urinib ko'ring!")
    }
