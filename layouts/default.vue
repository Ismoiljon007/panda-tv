<template>
    <header class="header" :class="{ 'header-fixed': scrolledNav }">
        <div class="container">
            ...
            <nav class="navigation">
                <ul class="navigation__list">
                    <li class="navigation__item" v-for="item in store.categories?.data?.categories" :key="item">
                        <NuxtLink :to="`/categorie/${item.slug}?page=1`">{{ item?.name }}</NuxtLink>
                    </li>
            ...
            <div class="header__second-nav">
                ...
                <NuxtLink to="/auth/login/" v-if="!authStore.token" class="header__login-btn">Kirish</NuxtLink>
                <button class="header__profile" v-else @click="profile = !profile">
                    ...
                    <Transition name="fade">
                        <div class="profile-modal" v-if="profile">
                            ...
                            <NuxtLink to="/saved" class="profile-modal__item"><img
                                    src="@/assets/images/svg/bookmark.svg" alt="">
                                Tanlanganlar</NuxtLink>
                            <NuxtLink to="/history" class="profile-modal__item"><img
                                    src="@/assets/images/svg/history.svg" alt="">
                                Tarix</NuxtLink>
    ...
    <transition name="menu">
        <div class="menu" v-if="menu">
            ...
            <NuxtLink v-if="!authStore.token" to="/auth/login/" class="btn">Kirish</NuxtLink>
        </div>
    </transition>
        ...
        <div class="search" v-if="store.search_open">
            ...
            <ul class="search-list" v-if="searchData?.length">
                <li v-for="item in searchData" :key="item">
                    <NuxtLink @click="store.search_open = false, store.overlay = false, searchData = []"
                        :to="item?.is_mobile_only == true ? '/only-on-mobile' : authStore.token == null ? '/auth/login/' : item?.type == 'Movie' ? `/movie/${item?.slug}/` : `/series/${item?.id}`">
    ...
    <footer class="footer">
        <div class="footer__top">
            <div class="container">
                <div class="footer-apps">
                    <a href="https://play.google.com/store/apps/details?id=com.app.pandaproduction" target="_blank">
                        <img src="@/assets/images/png/playmarket.png" alt="">
                    </a>
                    <a href="https://apps.apple.com/uz/app/panda-tv/id6476259906" target="_blank">
        ...
        <div class="footer__bottom">
            <div class="container">
                <div class="footer__desc">© {{ new Date().getFullYear() }} barcha huquqlar himoyalangan <a
                        href="https://pandatv.uz/">www.pandatv.uz</a></div>
                <ul>
                    <li>
