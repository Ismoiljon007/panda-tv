<template>
    <main>
                    ...
                    <div class="container">
                        <div class="hero__slide-text-wrapper">
                            ...
                            <NuxtLink class="hero__slide-btn"
                                :to="item?.is_mobile_only == true ? '/only-on-mobile' : authStore.token == null ? '/auth/login/' : item?.is_movie == true ? `/movie/${item.slug}` : `/series/${item.object_id}`">
        ...
        <div class="movies" v-for="(elem, i) in store.categories?.data?.categories" :key="elem.id">
            <div class="container">
                <div class="movies__header">
                    <NuxtLink :to="`/categorie/${elem.slug}?page=1`" class="movies__title">{{ elem?.name }}
                    </NuxtLink>
                    <NuxtLink :to="`/categorie/${elem.slug}?page=1`" class="view-all-category">Barchasi</NuxtLink>
                ...
                <Swiper :modules="[SwiperNavigation]"
                    ...
                    <SwiperSlide v-for="item in categorieMovies[`${elem?.id}`]" :key="item" class="movies__slide">
                        <ClientOnly>
        ...
        <div class="movies" v-if="megogoMovies?.data">
            <div class="container">
                <div class="movies__header">
                    <NuxtLink :to="`/megogo?page=1`" class="movies__title">Megogo kinolari</NuxtLink>
                    <NuxtLink :to="`/megogo?page=1`" class="view-all-category">Barchasi</NuxtLink>
                    <div class="movies__navigations">
                        <button :class="`movies__left movies-megogo__left`"><img src="@/assets/images/svg/left.svg"
                                alt=""></button>
                        <button :class="`movies__rigth movies-megogo__rigth`"><img src="@/assets/images/svg/right.svg"
                                alt=""></button>
                    </div>
                </div>
                <Swiper :modules="[SwiperNavigation]"
                    :navigation="{ nextEl: `.movies-megogo__rigth`, prevEl: `.movies-megogo__left` }"
                    :slides-per-view="'auto'" :space-between="30" :breakpoints="{
                        '650': {
                            spaceBetween: 30
                        },
                    ...
                    <SwiperSlide v-for="(movie) in megogoMovies?.data?.video_list" :key="movie.id"
                        class="movies__slide">
                        <ClientOnly>
...
const title: string = 'PandaTV.uz - Sevimli kontentingizni tomosha qiling va efirga uzating'
const description: string = 'PandaTV.uz sizning barcha oʻyin-kulgi ehtiyojlaringiz uchun bir platformaningizdir. Jonli efirlarni tomosha qiling, keng koʻlamli shoular va filmlar kutubxonasiga koʻring va har kuni yangi kontentni kashf eting.'
const keywords: any = ['pandatv', "o'zbek kinolar", 'uzbekistan', 'streaming', 'live tv', 'movies', 'shows']
...
async function getCategoryMovies() {
    try {
        store.loader = true
        const fetchPromises = store.categories?.data.categories.map(async (el: any) => {
            const data: any = await categorys.getCategoryMovies(el.slug, authStore.token, 1, 14)
