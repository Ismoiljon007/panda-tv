<template>
    <div class="movie-card">
        <div class="movie-card__img">
            ...
            <NuxtLink
                :to="props.movie?.is_mobile_only == true ? '/only-on-mobile' : authStore.token == null ? '/auth/login/' : props.movie?.is_movie == true || props.movie?.type == 'Movie' ? `/movie/${props.movie?.slug}/` : `/series/${props.movie?.id}?page=1`">
                <img class="img"
                    :src="props.movie?.thumbnail_image.includes(store.baseUrl) ? props.movie?.thumbnail_image : 'https://gateway.pandatv.uz' + props.movie?.thumbnail_image"
                    alt="">
        ...
        <div class="movie-card__badge-wrapper">
            <span class="movie-card__badge-quality">{{ props.movie?.is_free ? 'BEPUL' : 'OBUNA' }}</span>
            <span class="movie-card__badge-new" v-if="props.movie?.is_premiere">NEW</span>
        </div>

        <div class="movie-card__text-wrapper">
            <NuxtLink class="movie-card__title" :to="props.movie?.is_mobile_only == true ? '/only-on-mobile'
                : authStore.token == null ? '/auth/login/'
                    : props.movie?.is_movie == true ? `/movie/${props.movie?.slug}/`
                        : `/series/${props.movie?.id}?page=1`">
                {{ props.movie?.title }}
            </NuxtLink>
            <div class="movie-card__cat">
                <div>{{ yearGet(props.movie?.release_date) }}</div>
                <span>/</span>
                <div>{{ props.movie?.genre[0]?.name ? props.movie?.genre[0]?.name : props.movie?.genre[0] }}</div>
            </div>
        </div>
    </div>
</template>
...
async function addFvrt() {
    if (authStore.token) {
        if (props.movie?.is_movie) {
            if (activeSave.value) {
                await services.remFavorite(props.movie?.id, authStore.token, "SERIES", "movies")
                await getSavedMovies()
            } else {
                await services.addFavorite(props.movie?.id, authStore.token, "MOVIE", "movies")
                await getSavedMovies()
            }
        } else {
            if (activeSave.value) {
                await services.remFavorite(props.movie?.id, authStore.token, "SERIES", "series")
                await getSavedMovies()
            } else {
                await services.addFavorite(props.movie?.id, authStore.token, "SERIES", "series")
                await getSavedMovies()
            }
        }
    } else {
...
const activeSave = computed(() => {
    if (!authStore.token) {
        ...
        if (store.savedMovies?.data?.content.length) {
            const itemSave = toRaw(store.savedMovies?.data?.content)?.find((elem: any) => elem.id == props.movie.id)
            if (itemSave) {
                return true
            } else {
                return false
