        ...
        <div class="movie">
            <div class="container">
                ...
                <Swiper :modules="[SwiperNavigation]" :navigation="{ nextEl: '.movie__rigth', prevEl: '.movie__left' }"
                    ...
                    <SwiperSlide v-for="item in movies" :key="item"
                        :style="item?.id == Number(details?.data?.id) ? 'display: none;' : ''" class="movie__slide">
...
async function fetchData() {
    store.loader = true;
    try {
        const detailData: any = await movieServices.getMovieDetail(slug, authStore.token)
        ...
        // category movies
        const categorieDataMovie: any = await categorys.getCategoryMovies(detailData?.data?.category, authStore.token)
...
async function getDetilsComment() {
    try {
        const detailData: any = await await movieServices.getMovieDetail(slug, authStore.token)
        details.value = detailData;
    } catch (error) {
