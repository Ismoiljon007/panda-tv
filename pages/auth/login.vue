    ...
    <div class="login">
        ...
        <div class="login-text-wrapper">
            ...
            <form action="#" @submit.prevent="login()" class="login-form">
                <input autocomplete="false" v-model="phoneNumber" placeholder="+998" @input="mask" @focus="mask"
                    @blur="mask" @keydown="mask" class="phone login-input-tel">
                <div class="login-btns">
                    <button class="login-btn" type="submit">Ko'dni jo'natish</button>
                </div>
                <button type="button" class="login-google-sign" @click="loginWithEmail()">
...
const login = async () => {
    ...
    let tel = p.split('_').join('').length == 14 ? p.split('_').join('').slice(0, -1) : p.split('_').join('')
...
const mask = (event: any) => {
    ...
    let reg: any = matrix.substr(0, phoneNumber.value.length).replace(/_+/g,
        function (a) {
            return "\\d{1," + a.length + "}";
        }).replace(/[+()]/g, "\\$&");
    reg = new RegExp("^" + reg + "$");
