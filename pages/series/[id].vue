            ...
            <div class="container">
                <div class="movie-top">
                    <div class="movie-video" v-if="vidType == 'online'">
                        <video-player class="video-js video-player video-player-serie vjs-default-skin" controls
                            playsinline crossorigin :sources="[{
                                src: video_url,
                            ...
                            }" :playbackRates="[0.5, 1, 1.5, 2]" :poster="img_url" />
                    ...
                    <div class="movie-video" v-if="vidType == 'trailer'">
                        <ClientOnly>
                            <video-player-vue v-if="details?.data?.trailer_url" :item="{
                    ...
                    <div class="movie__info">
                        ...
                        <div class="movie__info-list">
                            <li style="display: flex;">Sanasi: <span style="margin-right: auto;">{{
                                details?.data?.release_date }}</span> Davomiyligi: <span
                        ...
                        <div class="movie__btns">
                            <button @click="vidType = 'online'"
                                ...
                                KO'RISH</button>
                            <button v-if="details?.data?.trailer_url" @click="vidType = 'trailer'"
                ...
                <div class="movie-episods" v-if="!details?.data?.is_movie">
                    <div class="movie-episods__seasons">
                        ...
                        <div class="movie-episods__seasons-item"
                            :class="item.id == details?.data?.seasons[0].id ? 'active-btn' : ''"
                    ...
                    <div class="movie-episods__wrapper">
                        <div class="movie-episods__item" :class="item.id == route.query.s ? 'active-item' : ''"
                ...
                <div class="pagination flex justify-center mb-8">
                    <vue-awesome-paginate :total-items="episods?.data?.pagination?.total_pages" :items-per-page="1"
                        ...
                        <ul class="movie__comments-item__inner">
                            <div class="movie__comments-reply" style="display: none;"
                                ...
                                <div class="movie__comments-text-wrapper">
                                    ...
                                    <button @click="replie(item?.id)"><img src="@/assets/images/svg/navigation.svg"
                                            alt="">
                                        Jo‘natish</button>
                                </div>
                            </div>
                                ...
                                <li class="movie__comments-item" v-for="el in item?.replies" :key="el">
                                    <div class="movie__comments-item-wrapper">
                                        ...
                                        <div class="movie__comments-item-text-wr">
                                            ...
                                            <p class="movie__comments-item-desc" style="margin-bottom: 0px;">{{
                                                el?.content
                                            }}</p>
                                            <div class="movie__comments-item-btns">
                                                <!-- <button>
...
async function getSeries(episode: number | string) {
    ...
    const data: any = await seriesApi.getSeaseons(id, episodeNumber.value, episode, authStore.token)
...
async function fetchData() {
    ...
    try {
        ...
        // category movies
        const categorieDataMovie: any = await categorys.getCategoryMovies(res?.data?.category, authStore.token)
...
const onClickHandler = async (page: number | string) => {
    ...
    const data: any = await $fetch(store.baseUrl + '/series/' + id + `/seasons/${episodeNumber.value}/episodes/?page=${page}`)
    episods.value = data
};
...
async function getWatchTime(id: number | string) {
    const res: any = await movieServices.getWatchTime(store.userInfo?.id, id, 'episode')
...
useSeoMeta({
    title: details.value?.data.title,
    ogTitle: details.value?.data.title,
    description: details.value?.data.description,
    ogDescription: details.value?.data.description,
    ogImage: '/favicon.ico',
})
watchEffect(() => {
    title.value
